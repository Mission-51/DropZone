...
public class PlayerAttack_Soldier : MonoBehaviour, IAttack
{
    ...
    void Awake()
    {
        ...
        // PhotonView가 필요: 원거리 공격(발사체) 및 스킬 동작을 네트워크 상에 동기화해야 함
    }
    ...
    public void GetAttackInput(bool fDown)
    {
        ...
        {
            StartAttack();
            // 공격 동작도 다른 클라이언트에 동기화되어야 하므로 PhotonView와 RPC 사용 필요
        }
    }
    ...
    public void UseSkill()
    {
        ...
        playerMovement.SetAttackState(true); // 공격 상태 전달
        playerMovement.TurnTowardsMouse(); // 공격 시작 시 마우스 방향으로 회전  
        anim.SetTrigger("doSkill"); // 스킬 애니메이션 실행
